...
// EndpointSlice name.
type spToEndpointMap map[api.ServicePortName]map[string]api.Endpoint
...
func standardEndpointInfo(ep *BaseEndpointInfo) api.Endpoint {
	return ep
}
...
func (cache *EndpointSliceCache) addEndpointsByIP(serviceNN types.NamespacedName, portNum int, endpointsByIP map[string]api.Endpoint, endpoints []*endpointInfo) map[string]api.Endpoint {
	if endpointsByIP == nil {
		endpointsByIP = map[string]api.Endpoint{}
	}
...
func endpointsMapFromEndpointInfo(endpointInfoBySP map[api.ServicePortName]map[string]api.Endpoint) EndpointsMap {
	...
	for svcPortName, endpointInfoByIP := range endpointInfoBySP {
		if len(endpointInfoByIP) > 0 {
			endpointsMap[svcPortName] = []api.Endpoint{}
...
func formatEndpointsList(endpoints []api.Endpoint) []string {
