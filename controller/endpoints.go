...
var _ api.Endpoint = &BaseEndpointInfo{}
...
func (info *BaseEndpointInfo) Equal(other api.Endpoint) bool {
...
type makeEndpointFunc func(info *BaseEndpointInfo) api.Endpoint
...
type UpdateEndpointMapResult struct {
	...
	StaleEndpoints []api.ServiceEndpoint
	// StaleServiceNames identifies if a service is stale.
	StaleServiceNames []api.ServicePortName
...
func (em EndpointsMap) Update(changes *EndpointChangeTracker) (result UpdateEndpointMapResult) {
	result.StaleEndpoints = make([]api.ServiceEndpoint, 0)
	result.StaleServiceNames = make([]api.ServicePortName, 0)
...
type EndpointsMap map[api.ServicePortName][]api.Endpoint
...
func (ect *EndpointChangeTracker) endpointsToEndpointsMap(endpoints *v1.Endpoints) EndpointsMap {
	...
	for i := range endpoints.Subsets {
		...
		for i := range ss.Ports {
			...
			svcPortName := api.ServicePortName{
				NamespacedName: types.NamespacedName{Namespace: endpoints.Namespace, Name: endpoints.Name},
				Port:           port.Name,
...
func (em EndpointsMap) apply(ect *EndpointChangeTracker, staleEndpoints *[]api.ServiceEndpoint,
	staleServiceNames *[]api.ServicePortName, lastChangeTriggerTimes *map[types.NamespacedName][]time.Time) {
	if ect == nil {
		return
	}
...
func detectStaleConnections(oldEndpointsMap, newEndpointsMap EndpointsMap, staleEndpoints *[]api.ServiceEndpoint, staleServiceNames *[]api.ServicePortName) {
	for svcPortName, epList := range oldEndpointsMap {
		...
		for _, ep := range epList {
			...
			if stale {
				...
				*staleEndpoints = append(*staleEndpoints, api.ServiceEndpoint{Endpoint: ep.String(), ServicePortName: svcPortName})
			}
		}
	}
