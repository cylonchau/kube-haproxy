...
var _ api.ServicePort = &BaseServiceInfo{}
...
type makeServicePortFunc func(*v1.ServicePort, *v1.Service, *BaseServiceInfo) api.ServicePort
...
func NewServiceChangeTracker(makeServiceInfo makeServicePortFunc, recorder record.EventRecorder, processServiceMapChange processServiceMapChangeFunc) *ServiceChangeTracker {
...
type ServiceMap map[api.ServicePortName]api.ServicePort
...
func (sct *ServiceChangeTracker) serviceToServiceMap(service *v1.Service) ServiceMap {
	...
	for i := range service.Spec.Ports {
		...
		svcPortName := api.ServicePortName{NamespacedName: svcName, Port: servicePort.Name, Protocol: servicePort.Protocol}
...
func (sm *ServiceMap) merge(other ServiceMap) sets.String {
	...
	for svcPortName, info := range other {
		...
		if !exists {
			klog.V(3).Infof("Adding new service port %q at %s", svcPortName, info.String())
		} else {
			klog.V(3).Infof("Updating existing service port %q at %s", svcPortName, info.String())
		}
		(*sm)[svcPortName] = info
	}
